...
void UUnitAnimInstance::UnitAnimInit(UUnitAgent* InitParent, FLegIKProfile InitProfile) {
...
FLegIKDynamics UUnitAnimInstance::UnitAnimComputeLegDynamics() {
    // TODO: Impl.
    return FLegIKDynamics();
}

// TODO: Optimize and refactor.
void UUnitAnimInstance::UnitAnimTick(float RawDeltaTime, USceneComponent* Parent) {
    // TODO: Move legdyn lower and compute all script props by inspecting parent.
    FLegIKDynamics LegDynamics = this->UnitAnimComputeLegDynamics();

    FVector BodyLocation = Parent->GetComponentLocation();
    FRotator BodyRotation = Parent->GetComponentRotation();
    FVector BodyVelocity = LegDynamics.BodyVelocity;
    ...
    for (int i = 0; i < ActiveStepGroups.Num(); i++) {
        ...
        for (int j = 0; j < CheckGroup->Tracks.Num(); j++) {
            ...
            bool Invalidated = (
                CheckTrack->TrackStepErrorDistance(BodyVelocity) >
                (CurrentOffsetTolerance * 2.0f)
            );
    ...
    if (ConsiderStep) {
        ...
        for (int i = 0; i < this->TrackGroups.Num(); i++) {
            ...
            float CheckOffset = FMath::Abs((
                CheckGroup->Tracks[0].TrackCurrentLocation() -
    ...
    for (int i = 0; i < this->TrackGroups.Num(); i++) {
        ...
        for (int j = 0; j < Group->Tracks.Num(); j++) {
            FVector TargetWorldLocation = Group->Tracks[j].LegIKTrackTick(DeltaTime, Parent, LegDynamics);
    ...
    FVector TickRootTarget = FVector(
        0.0f, 0.0f,
        (this->Profile.BodyBaseOffset) * LegDynamics.BodyBaseCoef
    );

    FVector InterpedRootTarget = FMath::VInterpTo(
        LastRootTarget, TickRootTarget,
        DeltaTime,
        this->Profile.LerpRate * LegDynamics.BodyLerpRateCoef
    );
